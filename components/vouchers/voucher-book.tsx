...
export function VoucherBook({ selectedCompany }: { selectedCompany: string }) {
  ...
  const filteredEntries = useMemo(() => entries.filter((entry) => matchesCompanyFilter(entry.company, companyFilter) && (selectedFilter === "All" ? true : entry.category === selectedFilter) && isDateWithinRange(entry.date, fromDate, toDate)), [entries, companyFilter, selectedFilter, fromDate, toDate]);
