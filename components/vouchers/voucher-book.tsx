...
export function VoucherBook({ selectedCompany }: { selectedCompany: string }) {
  ...
  const [entries, setEntries] = useState<VoucherEntry[]>(voucherEntries);
  ...
  const filteredEntries = useMemo(() => entries.filter((entry) => matchesCompanyScope(entry.company, companyScope) && (selectedFilter === "All" ? true : entry.category === selectedFilter) && isDateWithinRange(entry.date, fromDate, toDate)), [entries, companyScope, selectedFilter, fromDate, toDate]);

  const entryCompany = companyScope === ALL_COMPANIES ? "Vishnu Bankers - Main Branch" : companyScope;

  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const nextEntry: VoucherEntry = { id: `voucher-${Date.now()}`, date: voucherDate, company: entryCompany, category, payee: payee || "Expense entry", remarks, amount: Number(amount) || 0 };
    setEntries((current) => [nextEntry, ...current]);
    setStatusMessage(`${category} voucher recorded for ${entryCompany}. Ledger linkage will be persisted in the backend layer.`);
    ...
    <div className="space-y-6">
      <form onSubmit={handleSubmit} className="app-panel rounded-[30px] p-6 sm:p-8">
        ...
        <div className="mt-6 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
          ...
          <label className="block space-y-2 xl:col-span-3"><span className="text-sm font-medium text-[var(--color-muted)]">Company</span><input value={entryCompany} readOnly className="w-full rounded-2xl border border-[var(--color-border)] bg-[var(--color-page)] px-4 py-3 text-[var(--color-muted)]" /></label>
