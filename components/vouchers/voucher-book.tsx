...
export function VoucherBook({ selectedCompany }: { selectedCompany: string }) {
  ...
  const [statusMessage, setStatusMessage] = useState("Loading vouchers from Supabase...");
  ...
  useEffect(() => {
    ...
    async function loadEntries() {
      try {
        ...
        if (!response.ok) {
          setStatusMessage(result.error ?? "Unable to load vouchers.");
          return;
        }
        setEntries((result.vouchers ?? []) as VoucherEntry[]);
        setStatusMessage("Vouchers are now using real Supabase data and posting to the ledger.");
      } catch {
        if (isMounted) {
          setStatusMessage("Unable to reach the voucher endpoint.");
        }
      }
    }
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setIsSaving(true);
    setStatusMessage("Saving voucher to Supabase...");
    ...
    try {
      ...
      if (!response.ok) {
        setStatusMessage(result.error ?? "Unable to save voucher.");
        return;
      }

      if (result.voucher) {
        setEntries((current) => [result.voucher as VoucherEntry, ...current]);
      }
      setStatusMessage(result.message ?? `${category} voucher recorded successfully.`);
      setPayee("");
      setRemarks("");
      setAmount("0");
