...
export function VoucherBook({ selectedCompany }: { selectedCompany: string }) {
  const [entries, setEntries] = useState<VoucherEntry[]>(
    voucherEntries.filter((entry) => entry.company === selectedCompany),
  );
  const [voucherDate, setVoucherDate] = useState(getTodayDisplayDate());
  ...
  const [statusMessage, setStatusMessage] = useState(
    "Voucher section is ready. Expense categories recorded here will feed the ledger once backend save is wired.",
  );
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setStatusMessage(
      `${category} voucher recorded for ${selectedCompany}. Ledger linkage will be persisted in the backend layer.`,
    );
    setPayee("");
    setRemarks("");
    setAmount("0");
    ...
    <div className="space-y-6">
      <form onSubmit={handleSubmit} className="app-panel rounded-[30px] p-6 sm:p-8">
        ...
        <div className="mt-6 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
          ...
          <label className="block space-y-2">
            ...
            <select
              value={category}
              onChange={(event) => setCategory(event.target.value as VoucherEntry["category"])}
              className="w-full rounded-2xl border border-[var(--color-border)] bg-white px-4 py-3 outline-none transition focus:border-[var(--color-accent)]"
            >
          ...
          <label className="block space-y-2">
            <span className="text-sm font-medium text-[var(--color-muted)]">Payee</span>
            <input
              value={payee}
          ...
          <label className="block space-y-2">
            <span className="text-sm font-medium text-[var(--color-muted)]">Amount</span>
            <input
              value={amount}
          ...
          <label className="block space-y-2 xl:col-span-4">
        ...
        <section className="mt-6 rounded-[24px] border border-[var(--color-border)] bg-white p-5">
      ...
      <section className="app-panel rounded-[30px] p-6 sm:p-8">
        <div className="flex flex-wrap gap-2">
          {(["All", ...voucherCategories] as const).map((item) => (
            <button
              key={item}
              type="button"
        ...
        <div className="mt-6 grid gap-3">
          {filteredEntries.map((entry) => (
            ...
            >
              <div className="grid gap-3 lg:grid-cols-[0.12fr_0.18fr_0.18fr_0.16fr_0.36fr] lg:items-center">
                <Cell label="Date" value={entry.date} />
                <Cell label="Category" value={entry.category} />
                <Cell label="Payee" value={entry.payee} />
