...
export function PaymentAdjustmentsBook() {
  const [adjustments] = useState<PaymentAdjustmentRecord[]>(() => loadPaymentAdjustments());
  ...
  const filteredAdjustments = useMemo(() => {
    return adjustments.filter((adjustment) => {
      const matchesQuery = query
        ? [
            adjustment.loanAccountNumber,
            adjustment.customerName,
            adjustment.reason,
            adjustment.acknowledgedBy,
          ]
            .join(" ")
            .toLowerCase()
    ...
    <div className="space-y-6">
      <section className="app-panel rounded-[30px] p-6 sm:p-8">
        ...
        <div className="mt-6 grid gap-4 xl:grid-cols-[minmax(260px,1fr)_240px_220px] xl:items-center">
          <div className="flex items-center gap-3 rounded-2xl border border-[var(--color-border)] bg-white px-4 py-4 text-sm text-[var(--color-muted)]">
            ...
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search by loan, customer, reason, or staff name" className="w-full bg-transparent outline-none" />
          </div>
          ...
          <select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value as PaymentAdjustmentType | "All")} className="w-full rounded-2xl border border-[var(--color-border)] bg-white px-4 py-4 text-sm outline-none transition focus:border-[var(--color-accent)]">
      ...
      <section className="app-panel rounded-[30px] p-6 sm:p-8">
        <div className="overflow-x-auto pb-2">
          <div className="grid min-w-[1360px] gap-3">
            <div className="hidden rounded-[24px] bg-[var(--color-panel-strong)] px-5 py-4 text-sm font-semibold text-[var(--color-ink)] lg:grid lg:grid-cols-[minmax(150px,0.9fr)_minmax(220px,1.2fr)_minmax(190px,1.2fr)_minmax(140px,0.9fr)_minmax(140px,0.9fr)_minmax(140px,0.8fr)_minmax(120px,0.8fr)_minmax(140px,0.9fr)] lg:items-center">
              <span>Date</span>
              <span>Loan / Customer</span>
              <span>Reason</span>
              <span>Type</span>
              <span>Principal delta</span>
              <span>Interest delta</span>
              <span>Status</span>
              <span>Staff</span>
            </div>
            ...
            {filteredAdjustments.map((adjustment) => (
              <article key={adjustment.id} className="rounded-[24px] border border-[var(--color-border)] bg-white px-5 py-4">
                <div className="grid gap-3 lg:grid-cols-[minmax(150px,0.9fr)_minmax(220px,1.2fr)_minmax(190px,1.2fr)_minmax(140px,0.9fr)_minmax(140px,0.9fr)_minmax(120px,0.8fr)_minmax(120px,0.8fr)_minmax(140px,0.9fr)] lg:items-center">
                  <Cell label="Date" value={adjustment.createdAt} subValue={adjustment.company} />
                  <Cell label="Loan / Customer" value={adjustment.loanAccountNumber} subValue={adjustment.customerName} strong />
                  <Cell label="Reason" value={adjustment.reason} />
                  <Cell label="Type" value={adjustment.correctionType} />
                  <Cell label="Principal delta" value={formatAdjustmentDelta(adjustment.principalAdjustment)} />
                  <Cell label="Interest delta" value={formatAdjustmentDelta(adjustment.interestAdjustment)} />
                  <div><p className="text-xs uppercase tracking-[0.14em] text-[var(--color-muted)] lg:hidden">Status</p><span className="inline-flex rounded-full bg-amber-100 px-3 py-1 text-xs font-medium uppercase tracking-[0.14em] text-amber-900">{adjustment.status}</span></div>
                  <Cell label="Staff" value={adjustment.acknowledgedBy} />
                </div>
              </article>
            ))}
          </div>
        ...
        {!filteredAdjustments.length ? (
          <div className="mt-6 rounded-[24px] border border-dashed border-[var(--color-border)] bg-white px-5 py-8 text-center text-sm text-[var(--color-muted)]">
            No payment corrections found for the selected filters.
          </div>
        ) : null}
      </section>
      ...
      <section className="app-panel rounded-[30px] p-6 sm:p-8">
        <div className="flex items-center gap-3 text-[var(--color-accent)]">
          ...
          <p className="text-sm font-semibold uppercase tracking-[0.18em]">Control Note</p>
        </div>
        <p className="mt-4 max-w-4xl text-sm leading-7 text-[var(--color-muted)]">
          Original payments are preserved. Any correction should be posted as a linked reversal or adjustment entry with reason and staff acknowledgement, so audit review can always trace the exact change path.
        </p>
      </section>
    </div>
  );
