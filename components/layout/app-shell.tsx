    ...
    <div className="h-screen overflow-hidden p-4 sm:p-6">
      <div className="mx-auto grid h-[calc(100vh-2rem)] max-w-7xl overflow-hidden rounded-[32px] border border-[var(--color-border)] bg-[rgba(255,255,255,0.55)] shadow-[0_40px_90px_rgba(54,39,19,0.15)] lg:grid-cols-[280px_1fr]">
        <aside className="flex h-full flex-col justify-between overflow-y-auto bg-[linear-gradient(180deg,#1f2937_0%,#243447_45%,#312217_100%)] p-6 text-white sm:p-7">
          ...
          <div className="rounded-[24px] border border-white/10 bg-white/8 p-5"><p className="text-xs uppercase tracking-[0.18em] text-white/55">Active company</p><p className="mt-2 text-lg font-semibold">{selectedCompany}</p></div>
        </aside>
        <div className="flex h-full min-h-0 flex-col overflow-hidden"><header className="relative flex shrink-0 items-center justify-between border-b border-[var(--color-border)] px-5 py-5 sm:px-8"><div className="min-w-[220px] text-sm font-medium text-[var(--color-ink)]">{backLink ? <Link href={backLink.href} className="inline-flex items-center gap-2 transition hover:text-[var(--color-accent-strong)]"><ArrowLeft className="h-4 w-4" />{backLink.label}</Link> : null}</div><div className="absolute left-1/2 top-1/2 w-[320px] max-w-[42vw] -translate-x-1/2 -translate-y-1/2"><select value={selectedCompany} onChange={(event) => handleCompanyChange(event.target.value)} className="w-full appearance-none rounded-full border border-[var(--color-border)] bg-white/90 px-5 py-2 text-sm font-semibold text-[var(--color-ink)] shadow-sm outline-none transition focus:border-[var(--color-accent)]">{companyOptions.map((company) => <option key={company.code} value={company.name}>{company.name}</option>)}</select><ChevronDown className="pointer-events-none absolute right-4 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--color-muted)]" /></div><div className="flex items-center gap-3"><div className="flex min-w-[220px] items-center gap-3 rounded-2xl border border-[var(--color-border)] bg-white px-4 py-3 text-sm text-[var(--color-muted)]"><Search className="h-4 w-4" /><span>Search records</span></div><div className="rounded-2xl bg-[var(--color-sidebar)] px-4 py-3 text-sm font-medium text-white">Admin</div></div></header><main className="min-h-0 flex-1 overflow-y-auto px-5 py-6 sm:px-8">{children}</main></div>
      </div>
    </div>
  );
}
