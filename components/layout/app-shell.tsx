      ...
      <div className="mx-auto grid h-[calc(100vh-1rem)] max-w-[1720px] overflow-hidden rounded-[32px] border border-[var(--color-border)] bg-[rgba(255,255,255,0.55)] shadow-[0_40px_90px_rgba(54,39,19,0.15)] lg:grid-cols-[248px_1fr]">
        <aside className="flex h-full flex-col justify-between overflow-y-auto bg-[linear-gradient(180deg,#1f2937_0%,#243447_45%,#312217_100%)] p-5 text-white sm:p-6">
          <div>
            ...
            <button type="button" onClick={() => setShowInterestCalculator((current) => !current)} className="mt-6 flex w-full items-center justify-between rounded-2xl border border-white/10 bg-white/8 px-4 py-3 text-sm font-medium text-white/88 transition hover:bg-white/12 hover:text-white"><span className="flex items-center gap-3"><Calculator className="h-4 w-4 text-amber-200" /><span>Interest Calculator</span></span><ChevronDown className={`h-4 w-4 transition ${showInterestCalculator ? "rotate-180" : "rotate-0"}`} /></button>
            {showInterestCalculator ? <div className="mt-3 rounded-[24px] border border-white/10 bg-white/8 p-4"><p className="text-[11px] uppercase tracking-[0.18em] text-white/55">Annual Simple Interest</p><div className="mt-3 space-y-3"><label className="block"><span className="mb-1.5 block text-xs text-white/65">Principal</span><input value={calculatorPrincipal} onChange={(event) => setCalculatorPrincipal(event.target.value)} placeholder="0" className="w-full rounded-2xl border border-white/10 bg-black/10 px-3 py-2.5 text-sm text-white outline-none transition placeholder:text-white/35 focus:border-amber-200/70" /></label><label className="block"><span className="mb-1.5 block text-xs text-white/65">Rate % per year</span><input value={calculatorRate} onChange={(event) => setCalculatorRate(event.target.value)} placeholder="0" className="w-full rounded-2xl border border-white/10 bg-black/10 px-3 py-2.5 text-sm text-white outline-none transition placeholder:text-white/35 focus:border-amber-200/70" /></label><label className="block"><span className="mb-1.5 block text-xs text-white/65">Years</span><input value={calculatorYears} onChange={(event) => setCalculatorYears(event.target.value)} placeholder="1" className="w-full rounded-2xl border border-white/10 bg-black/10 px-3 py-2.5 text-sm text-white outline-none transition placeholder:text-white/35 focus:border-amber-200/70" /></label></div><div className="mt-4 grid gap-2"><div className="rounded-2xl border border-white/10 bg-black/10 px-3 py-2.5"><p className="text-[11px] uppercase tracking-[0.16em] text-white/50">Interest</p><p className="mt-1 text-sm font-semibold text-white">Rs {calculatorInterest.toFixed(2)}</p></div><div className="rounded-2xl border border-white/10 bg-black/10 px-3 py-2.5"><p className="text-[11px] uppercase tracking-[0.16em] text-white/50">Total Amount</p><p className="mt-1 text-sm font-semibold text-white">Rs {calculatorTotal.toFixed(2)}</p></div></div></div> : null}
