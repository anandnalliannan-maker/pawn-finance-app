...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => { setOpenSection(getSectionFromPath(pathname)); }, [pathname]);
  ...
  return (
    <div className="h-screen overflow-hidden p-4 sm:p-6">
      <div className="mx-auto grid h-[calc(100vh-2rem)] max-w-7xl overflow-hidden rounded-[32px] border border-[var(--color-border)] bg-[rgba(255,255,255,0.55)] shadow-[0_40px_90px_rgba(54,39,19,0.15)] lg:grid-cols-[280px_1fr]">
        ...
        <div className="flex h-full min-h-0 flex-col overflow-hidden"><header className="relative flex shrink-0 items-center justify-between border-b border-[var(--color-border)] px-5 py-5 sm:px-8"><div className="min-w-[220px] text-sm font-medium text-[var(--color-ink)]">{backLink ? <Link href={backLink.href} className="inline-flex items-center gap-2 transition hover:text-[var(--color-accent-strong)]"><ArrowLeft className="h-4 w-4" />{backLink.label}</Link> : null}</div><div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full border border-[var(--color-border)] bg-white/90 px-5 py-2 text-sm font-semibold text-[var(--color-ink)] shadow-sm">{selectedCompany}</div><div className="flex items-center gap-3"><div className="flex min-w-[220px] items-center gap-3 rounded-2xl border border-[var(--color-border)] bg-white px-4 py-3 text-sm text-[var(--color-muted)]"><Search className="h-4 w-4" /><span>Search records</span></div><div className="rounded-2xl bg-[var(--color-sidebar)] px-4 py-3 text-sm font-medium text-white">Admin</div></div></header><main className="min-h-0 flex-1 overflow-y-auto px-5 py-6 sm:px-8">{children}</main></div>
      </div>
    </div>
  );
}
