...
export function DepositCreationForm({ selectedCompany }: DepositCreationFormProps) {
  ...
  const [statusMessage, setStatusMessage] = useState("Deposit form is connected to Supabase save.");
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        setStatusMessage(result.error ?? "Unable to save deposit.");
        return;
      }
      setDepositorName("");
      setPhoneNumber("+91 ");
      ...
      setStatusMessage(result.message ?? "Deposit saved successfully.");
    } catch {
      setStatusMessage("Unable to reach the deposit save endpoint.");
    } finally {
      setIsSaving(false);
    }
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <section className="app-panel rounded-[30px] p-6 sm:p-8">
        ...
        <div className="mt-6 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
          <div className="rounded-[24px] border border-[var(--color-border)] bg-white px-4 py-3"><p className="text-xs uppercase tracking-[0.14em] text-[var(--color-muted)]">Depositor ID</p><p className="mt-2 text-sm font-medium text-[var(--color-ink)]">Auto</p></div>
      ...
      <section className="app-panel rounded-[30px] p-6 sm:p-8"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="flex items-start gap-3 text-sm text-[var(--color-muted)]"><AlertCircle className="mt-1 h-4 w-4 text-[var(--color-accent)]" /><p className="max-w-3xl leading-7">{statusMessage}</p></div><button type="submit" disabled={isSaving} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-[var(--color-accent)] px-5 py-3 text-sm font-semibold text-white transition hover:bg-[var(--color-accent-strong)] disabled:cursor-not-allowed disabled:opacity-70"><Save className="h-4 w-4" />{isSaving ? "Saving..." : "Save Deposit F7"}</button></div></section>
    </form>
  );
}
