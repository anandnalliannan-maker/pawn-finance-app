...
export function DepositCreationForm({ selectedCompany }: DepositCreationFormProps) {
  ...
  const [depositDate, setDepositDate] = useState(getTodayDisplayDate());
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setStatusMessage(
      `Deposit draft captured for ${depositorName || "new depositor"} under ${selectedCompany}.`,
    );
  }
      ...
      <section className="app-panel rounded-[30px] p-6 sm:p-8">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-semibold text-[var(--color-ink)]">Create deposit</h1>
          </div>
        ...
        <div className="mt-6 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
          <div className="rounded-[24px] border border-[var(--color-border)] bg-white px-4 py-3">
            <p className="text-xs uppercase tracking-[0.14em] text-[var(--color-muted)]">Depositor ID</p>
            <p className="mt-2 text-sm font-medium text-[var(--color-ink)]">{nextDepositorCode}</p>
          </div>
          ...
          <label className="block space-y-2 md:col-span-2">
            <span className="text-sm font-medium text-[var(--color-muted)]">Depositer Name</span>
            <input
              value={depositorName}
          ...
          <label className="block space-y-2 xl:col-span-2">
            <span className="text-sm font-medium text-[var(--color-muted)]">Address</span>
            <textarea
              value={address}
          ...
          <label className="block space-y-2">
            <span className="text-sm font-medium text-[var(--color-muted)]">Deposit Amount</span>
            <input
              value={depositAmount}
